...
function getReserves(req, res){
   ...
   Reserve.find({'date':new Date(Date.now()).getUTCFullYear().toString() + '-' + (new Date(Date.now()).getUTCMonth() +1) + '-'+ new Date(Date.now()).getUTCDate()+ 'T00:00:00.000Z'}).sort('time').exec(function(err,reserves,total){
    if(err){
        res.status(500).send({message: "Error en la peticion"});
    }else{
        if(!reserves){
            ...
            var x = new Date(Date.now()).getUTCFullYear().toString() + '-' + (new Date(Date.now()).getUTCMonth() +1) + '-'+ new Date(Date.now()).getUTCDate()+ 'T00:00:00.000Z';
